...
class SegmentationDataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		# Adjusting pixel values to 0 and 1 for binary mask
		# im = im.astype(np.float32) / 255.0  # Normalize to [0, 1] range
		lbl = lbl.astype(np.float32) / 255.0  # Normalize to [0, 1] range
		...
		try:
			...
			for im_fn, seg_fn in tqdm(self.pairs_dir, ncols=TQDM_COLS):
				...
				if not img.shape == seg.shape:
					...
					max_pixel_value = np.max(seg[:, :, 0])/255
