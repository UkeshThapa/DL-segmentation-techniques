...
class Trainer(object):
    ...
    def __init__(self,epoch,best_metrics,model_name,model, optimizer, logger, num_epochs, train_loader,
                 test_loader=None,
                 log_batch_stride=30,
        ...
        print(f"The best metrics : {self.best_metrics}")
        ...
        for n_batch, (sample_batched) in tqdm(enumerate(self.train_loader),
            ...
            if self.test_loader:
                ...
                with torch.no_grad():
                    ...
                    metrics = self._eval_batch(sample_batched, n_batch, num_batches)
            
                    # modify by UT comparing mean iou
                    if metrics > self.best_metrics:
                        self.best_metrics = metrics
                        best_metric_epoch = self.epoch + 1
                        state ={
                            'epoch' : best_metric_epoch,
                            'state_dict' : self.model.state_dict(),
                            'optimizer' : self.optim.state_dict(),
                            'best_metric' : self.best_metrics
                        ...
                        torch.save(state, f"..\\result\model_{self.model_name}.pth.tr")
                        
                        print(
                            "\nSaved new best metric model"
                            f"\nBest mean IOU: {self.best_metrics:.4f} "
